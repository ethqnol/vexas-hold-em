...
import { LogIn, LogOut, Loader2, ShieldAlert, Wallet, Trophy } from 'lucide-react';
...
function App() {
  ...
  const fetchUserProfile = async (uid: string) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/users/${uid}`);
      if (res.ok) {
  ...
  const fetchUserPortfolio = async (uid: string) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/users/${uid}/portfolio`);
      if (res.ok) {
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        try {
          await fetch(`${import.meta.env.VITE_API_URL}/api/v1/users/${currentUser.uid}/sync`, {
            method: 'POST',
  ...
  useEffect(() => {
    if (profile?.Cosmetic) {
      document.body.className = profile.Cosmetic;
    } else {
  ...
  return (
    <>
      <Routes>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="hidden sm:block text-sm text-gray-400 text-right">
                        <div className="flex items-center gap-2">
                          ...
                          {profile && (
                            <span className={`text-xs px-1.5 py-0.5 rounded-md ${rank.bgClass} border ${rank.borderClass} ${rank.colorClass} font-semibold uppercase tracking-tighter`}>
                        ...
                        {profile && (
                          <div className="text-green-400 font-mono text-xs font-bold">
                            ...
                            <span className="text-[10px] text-green-600 ml-1 opacity-70">S.H.I.T.</span>
                          </div>
                        )}
                      </div>
                      <Link
                        to="/leaderboard"
                        className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-yellow-400 bg-yellow-500/10 hover:bg-yellow-500/20 rounded-lg transition-colors border border-yellow-500/20 shadow-lg shadow-yellow-500/5"
                      >
        ...
        <Route path="/casino/roulette" element={<Roulette user={user} profile={profile} refreshProfile={refreshProfile} />} />
        <Route path="/casino/slots" element={<Slots user={user} profile={profile} refreshProfile={refreshProfile} />} />
        <Route path="/casino/blackjack" element={<Blackjack user={user} profile={profile} refreshProfile={refreshProfile} />} />
        <Route path="/casino/lottery" element={<Lottery user={user} profile={profile} refreshProfile={refreshProfile} />} />
        <Route path="/casino/store" element={<Store user={user} profile={profile} refreshProfile={refreshProfile} />} />
